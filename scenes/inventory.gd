...
func inventory_map(item : Item, amount : int):
	if inventory.has(item):
		inventory[item] += amount
		return
	inventory[item] = amount

func check_ingredients(ingredients : Dictionary[Item, int]):
	for key in ingredients:
		if !inventory.has(key) or ingredients[key] < ingredients[key]:
			return false
	return true
...
func use_ingredients(ingredients : Dictionary[Item, int]):
	for key in ingredients:
